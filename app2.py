...
def page_cards():
    ...
    with st.expander("➕ Add"):
        ...
        with col1:
            st.text_input("Name*", placeholder="Nubank")
            st.selectbox("Brand", ["Visa","Mastercard","Elo","Amex","Other"])
        with col2:
            st.text_input("Last 4", max_chars=4)
            st.number_input("Limit (R$)*", min_value=0.0, step=100.0)
        with col3:
            st.number_input("Closing day*", 1, 31, 2)
            st.number_input("Due day*", 1, 31, 10)
        st.color_picker("Color", "#10B981")
        if st.button("Save", type="primary"): st.success("Saved (demo)")
...
def page_transactions():
    ...
    with st.expander("➕ Add"):
        ...
        if st.button("Save transaction", type="primary"): st.success("Saved (demo)")
